...
Rezultati pokazuju da druga arhitektura postiže 77.55\% top-1 točnosti na 261 klasi, što je poboljšanje od 5.25\% u odnosu na originalnu osnovicu (72.30\%) uz 52\% manje parametara.
...
Primjerice, ResNet-50 zahtijeva 98\,MB pohrane i više od 3.8 milijarde FLOPs za obradu jedne slike.
...
Analiza originalnog modela pokazala je značajan overfitting: točnost na treningu doseže približno 99\% dok točnost na testu dosegnula je plato na 72\%.
Razlika od približno 27\% ukazuje na to da model previše ovisi o memorizaciji trening skupa.
...
\begin{enumerate}
  ...
  \item \textbf{Treći konvolucijski blok (16$\rightarrow$32 filtera)} -- dodaje dubinu i treći MaxPool sloj koji smanjuje prostornu dimenziju s $25\times25$ na $12\times12$. Time se ulaz u FC1 reducira s 10 000 na 4 608 značajki, a sam FC1 sa 1.28M na 590K parametara.
\end{enumerate}
...
Za razliku od klasične paradigme \textit{train $\rightarrow$ prune $\rightarrow$ fine-tune}, ovdje primijenjeni dinamički pruning postupno povećava sparsity tijekom treniranja prema rasporedu:
...
\begin{enumerate}
  \item \textbf{Treniranje s dinamičkim pruningom}: \textit{SimpleCNN} trenira se 15 epoha s $\eta = 0.01$, batch size 32, momentum 0.9. Pruning počinje u 2. epohi i postupno postiže 50\% sparsity do 15. epohe.
...
Poboljšana arhitektura postiže poboljšanje od 5.25\% u točnosti uz istovremeno smanjenje broja parametara za 52.1\%.
...
\begin{table}[h]
\caption{Napredak dinamičkog pruninga tijekom treniranja poboljšanog modela.}
...
Poboljšani model bez pruninga postiže točnost od 77.55\%, dok je najbolji model postignut u 14. epohi s 77.45\% točnosti pri sparsity od 49.98\%.
...
\begin{table}[h]
\caption{Cjelovita usporedba svih faza kompresije u odnosu na originalni model (V1).}
...
{\scriptsize $^{**}$Teoretska veličina uz pohranu težina kao INT8 (1 bajt umjesto 4).}
\end{center}
\end{table}

Konačni komprimirani model je 16.7$\times$ manji od originalnog modela, dok istovremeno postiže 4.18\% bolju točnost (76.48\% vs 72.30\%).
...
Dinamički L1 pruning sa 50\% sparsity zadržao je točnost na 77.45\%.
...
Kombinacija sve tri tehnike daje model koji je 16.7$\times$ manji od originalne osnovice uz istovremeno 4.18\% bolju top-1 točnost.
Time je pokazano da s pažljivim dizajnom arhitekture u kombinaciji s dinamičkim pruningom i INT8 kvantizacijom moguće postići značajnu kompresiju bez značajnog gubitka prediktivne sposobnosti.
